...
namespace Start 
{
    public partial class Message : Form
    {
        ...
        {
            Queue q = new Queue(); a = new Queue(); string z;
            DataSet ds = new DataSet(),ds2=new DataSet(); int i = 0;ds2.ReadXml ("users.xml");
            ...
            while (i < dt.Rows.Count)
            {
                ...
                {
                    q.Enqueue(dt.Rows[i][2].ToString()); DataRow[] dr = dt2.Select("(([user] = '" +dt.Rows[i][1].ToString() + "'))");
                    z =dr[0]["Type"].ToString();
                   //done // dt.Rows[i][0].ToString() shu hwe l maalm yaane on va chercher dans users a la type dr[0]["type"] de hyda select user  avec cryptage et decryptage ll type
        ...
        private void Message_Load(object sender, EventArgs e)//load
        {
            ...
            q = ShowNewMessage(Variables.UserNom ,out a); int w = q.Count;//variables.username//done
            for (int i = 0; i < w; i++)
            {
        ...
        {
           TotaleMessage t = new TotaleMessage();t.Show();
        }
        public static void WriteMessage(string u1, string u2, string tm, string dateTime)//message
        {
            ...
            DataRow dr = dt.NewRow(); dr[0] = u1; dr[1] = u2;
        ...
        private void merssagesRecusToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (merssagesRecusToolStripMenuItem.Text == "Messages recus")
            {
                ...
                panel1.Visible = true; merssagesRecusToolStripMenuItem.Text = "Ecrire un message";
            }
             else
            {
                groupBox1.Visible =true ;
                panel1.Visible = false ; merssagesRecusToolStripMenuItem.Text = "Messages recus";
            }
        }
        ...
        private void Message_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            Variables.intro.Show(); Variables.intro.pictureBox2.Image = Properties.Resources.Msg;
        }
